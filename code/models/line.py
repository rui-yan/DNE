...
def line_loss(y_true, y_pred):
    return -tf.reduce_mean(tf.math.log(tf.sigmoid(y_true * y_pred)))

def create_model(numNodes, embedding_size, order='second'):
    v_i = tf.keras.layers.Input(shape=(1,))
    v_j = tf.keras.layers.Input(shape=(1,))

    first_emb = tf.keras.layers.Embedding(numNodes, embedding_size, name='first_emb')
    second_emb = tf.keras.layers.Embedding(numNodes, embedding_size, name='second_emb')
    context_emb = tf.keras.layers.Embedding(numNodes, embedding_size, name='context_emb')
    ...
    # First order
    first = tf.keras.layers.Lambda(lambda x: tf.reduce_sum(x[0] * x[1], axis=-1), name='first_order')([v_i_emb, v_j_emb])

    # Second order
    second = tf.keras.layers.Lambda(lambda x: tf.reduce_sum(x[0] * x[1], axis=-1), name='second_order')([v_i_emb_second, v_j_context_emb])
    ...
    model = tf.keras.models.Model(inputs=[v_i, v_j], outputs=prediction)
